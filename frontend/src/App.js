...
import { Activity, Radio, Cloud, Settings, PlayCircle, StopCircle, AlertCircle, CheckCircle, Send, FileText, Shield, Radar, Navigation, MapPin, Wifi, Battery, Zap, TrendingUp, AlertTriangle, Clock } from 'lucide-react';
import '@/App.css';
...
function App() {
  ...
  const fetch = async () => {
    try { const r = await axios.get(`${API}/gateway/status`); setSt(r.data); addEvent('STATUS', 'Updated'); } catch (e) {}
    try { const r = await axios.get(`${API}/gateway/config`); setCfg(r.data); } catch (e) {}
  ...
  return (
    <div style={{ background: C.p, color: C.txt, fontFamily: 'monospace', height: '100vh', overflow: 'hidden' }}>
      ...
      {v === 'dashboard' && <DenseBoard st={st} l={l} sc={sc} stxt={stxt} start={start} stop={stop} pub={pub} C={C} d={d} tgts={tgts} evts={evts} logs={logs} />}
      {v === 'config' && <Cfg cfg={cfg} C={C} />}
...
function DenseBoard({ st, l, sc, stxt, start, stop, pub, C, d, tgts, evts, logs }) {
  return (
    <div style={{ marginLeft: 64, padding: 8, height: 'calc(100vh - 60px)', overflow: 'auto' }}>
      ...
      <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(14, 1fr)', gap: 6 }}>
        ...
        <BtnC label="PUB" color={C.t} onClick={pub} disabled={!st?.is_running || l} C={C} />
        ...
        <div style={{ padding: '6px', background: C.s, border: `1px solid ${C.b}`, borderRadius: 3, textAlign: 'center', fontSize: 8 }}><Clock size={16} style={{ color: C.t }} /><div style={{ fontSize: 6, color: C.m }}>{new Date().getHours()}:{new Date().getMinutes()}</div></div>
        ...
        <div style={{ padding: '6px', background: C.s, border: `1px solid ${C.b}`, borderRadius: 3, textAlign: 'center', fontSize: 8, color: tgts.length > 0 ? C.e : C.ok }}><AlertTriangle size={16} /><div style={{ fontSize: 6, color: C.m }}>{tgts.length}</div></div>
      </div>
    </div>
  );
}
...
function Waveform({ C, st }) {
  ...
  useEffect(() => {
    ...
    const d = () => {
      ctx.fillStyle = C.s; ctx.fillRect(0, 0, w, h);
      ctx.strokeStyle = C.g; ctx.lineWidth = 0.3;
      for (let i = 0; i <= 3; i++) { ctx.beginPath(); ctx.moveTo(0, (h / 3) * i); ctx.lineTo(w, (h / 3) * i); ctx.stroke(); }
      if (st?.is_running) { data.current.shift(); data.current.push(Math.random()); }
      ctx.strokeStyle = C.t; ctx.lineWidth = 1.5;
      ctx.beginPath();
      data.current.forEach((p, i) => {
        const x = (i / data.current.length) * w, y = h / 2 + (p - 0.5) * h * 0.7;
...
function FreqSpec({ C, st }) {
  const ref = useRef(null);
  const data = useRef(Array(30).fill(0).map(() => Math.random()));
  useEffect(() => {
    const cv = ref.current; if (!cv) return;
    const ctx = cv.getContext('2d'), w = cv.width, h = cv.height, bars = 30;
    const d = () => {
      ctx.fillStyle = C.s; ctx.fillRect(0, 0, w, h);
      const bw = w / bars;
      if (st?.is_running) data.current = data.current.map(v => v * 0.95 + Math.random() * 0.05);
      data.current.forEach((v, i) => {
        const bh = v * h * 0.8;
        ctx.fillStyle = v > 0.7 ? C.e : v > 0.5 ? C.w : v > 0.3 ? C.t : C.t + '80';
...
function GeoMap({ C, tgts }) {
  const ref = useRef(null);
  useEffect(() => {
    const cv = ref.current; if (!cv) return;
    const ctx = cv.getContext('2d'), w = cv.width, h = cv.height, cx = w / 2, cy = h / 2, sc = 100;
    const d = () => {
      ctx.fillStyle = C.s; ctx.fillRect(0, 0, w, h);
      ctx.strokeStyle = C.g; ctx.lineWidth = 0.3;
      for (let i = 0; i <= 3; i++) {
      ...
      ctx.fillStyle = C.ok; ctx.beginPath(); ctx.arc(cx, cy, 6, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = C.ok; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(cx, cy, 10, 0, Math.PI * 2); ctx.stroke();
      tgts.forEach((t, idx) => {
        if (t.latitude && t.longitude) {
          const lat = parseFloat(t.latitude), lon = parseFloat(t.longitude);
          const x = cx + (lon - (-118.2437)) * sc, y = cy - (lat - 34.0522) * sc;
          const p = Math.sin(Date.now() / 200 + idx) * 3 + 8;
          ctx.fillStyle = C.e; ctx.beginPath(); ctx.moveTo(x, y - 6); ctx.lineTo(x - 4, y + 4); ctx.lineTo(x + 4, y + 4); ctx.closePath(); ctx.fill();
          ctx.strokeStyle = C.e; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.arc(x, y, p, 0, Math.PI * 2); ctx.stroke();
        }
      });
      requestAnimationFrame(d);
    };
...
function ThreatMtx({ C, tgts }) {
  const ref = useRef(null);
  useEffect(() => {
    const cv = ref.current; if (!cv) return;
    const ctx = cv.getContext('2d'), w = cv.width, h = cv.height, cols = 6, rows = 4, cw = w / cols, ch = h / rows;
    const d = () => {
      ctx.fillStyle = C.s; ctx.fillRect(0, 0, w, h);
      ctx.strokeStyle = C.b; ctx.lineWidth = 0.5;
      ...
      tgts.forEach(t => {
        const rm = t.range?.match(/([\d.]+)/), am = t.altitude?.match(/([\d.]+)/);
        if (rm && am) {
          const rng = parseFloat(rm[1]), alt = parseFloat(am[1]);
          const col = Math.min(cols - 1, Math.floor((rng / 50) * cols));
          const row = rows - 1 - Math.min(rows - 1, Math.floor((alt / 800) * rows));
          ctx.fillStyle = rng < 5 ? C.e : rng < 20 ? C.w : C.t + '60';
...
function AltChart({ C, tgts }) {
  const layers = ['>600m', '400-600', '200-400', '0-200'];
  const counts = layers.map((_, i) => tgts.filter(t => {
    const am = t.altitude?.match(/([\d.]+)/);
    if (!am) return false;
    const a = parseFloat(am[1]);
    return i === 0 ? a > 600 : i === 1 ? a >= 400 && a <= 600 : i === 2 ? a >= 200 && a < 400 : a < 200;
  }).length);
  return <div style={{ fontSize: 7 }}>{layers.map((l, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}><span style={{ color: C.m }}>{l}</span><div style={{ display: 'flex', gap: 1 }}>{[...Array(Math.max(0, counts[i]))].map((_, j) => <div key={j} style={{ width: 6, height: 8, background: C.t }} />)}</div></div>)}</div>;
}

function SpeedHist({ C, tgts }) {
  const ranges = ['0-50', '50-100', '100-150', '150+'];
  const counts = ranges.map((_, i) => tgts.filter(t => {
    const sm = t.speed?.match(/([\d.]+)/);
    if (!sm) return false;
    const s = parseFloat(sm[1]);
    return i === 0 ? s < 50 : i === 1 ? s >= 50 && s < 100 : i === 2 ? s >= 100 && s < 150 : s >= 150;
  }).length);
  return <div style={{ fontSize: 7 }}>{ranges.map((r, i) => <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}><span style={{ color: C.m }}>{r}kts</span><div style={{ display: 'flex', gap: 1 }}>{[...Array(Math.max(0, counts[i]))].map((_, j) => <div key={j} style={{ width: 6, height: 6, background: C.w }} />)}</div></div>)}</div>;
}

function Cfg({ cfg, C }) {
  return <div style={{ padding: 24 }}><Panel title="DARCY CONFIG" C={C} span={8}><div style={{ fontSize: 10, color: C.m }}>Configuration panel - Not implemented in dense view</div></Panel></div>;
}

function Logs({ logs, C }) {
  return <div style={{ padding: 24 }}><Panel title="LOGS" C={C} span={8}><div style={{ fontSize: 9, maxHeight: 400, overflowY: 'auto' }}>{logs.map((lg, i) => <div key={i} style={{ padding: 4, borderBottom: `1px solid ${C.b}` }}>{new Date(lg.timestamp).toLocaleString()} - {lg.data_type}</div>)}</div></Panel></div>;
}

function Panel({ title, children, C, span = 1, style = {} }) {
  return <div style={{ gridColumn: `span ${span}`, background: C.s, border: `1px solid ${C.b}`, borderRadius: 4, padding: 6, ...style }}><div style={{ fontSize: 8, fontWeight: 'bold', color: C.t, borderBottom: `1px solid ${C.b}`, paddingBottom: 3, marginBottom: 4 }}>{title}</div>{children}</div>;
}
